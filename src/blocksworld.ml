...
let action_reifier = MiniKanren.reify

type blocksWorldObject = A | B | C | D | E | F | G | X | Y

let show_obj = function
  | A -> "a"
  | B -> "b"
  | C -> "c"
  | D -> "d"
...
let obj_reifier = MiniKanren.reify

type blocksWorldPredicate = CLEAR | ONTABLE | HANDEMPTY | HOLDING | ON
...
let clear   x = atom !!true !!CLEAR (var_ !!x % nil())
let ontable x = atom !!true !!ONTABLE (var_ !!x % nil())
let handempty = atom !!true !!HANDEMPTY (nil())
let holding x = atom !!true !!HOLDING (var_ !!x % nil())
let on x y    = atom !!true !!ON (var_ !!x % (var_ !!y % nil()))

let not_clear   x = atom !!false !!CLEAR (var_ !!x % nil())
let not_ontable x = atom !!false !!ONTABLE (var_ !!x % nil())
let not_handempty = atom !!false !!HANDEMPTY (nil())
let not_holding x = atom !!false !!HOLDING (var_ !!x % nil())
let not_on x y    = atom !!false !!ON (var_ !!x % (var_ !!y % nil()))
